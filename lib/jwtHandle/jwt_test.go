...
func (s *S) TestInvalidAccessToken(c *C) {
	t := jwthandle.NewAccessToken(request, 3600)

	token, err := t.GenerateToken(params)
	c.Assert(err, IsNil)

	// assert extraction
	e, err := jwthandle.Extract(token.GetStringToken(), request.SignatureKey)
...
func (s *S) TestValidationWithWrongToken(c *C) {
	...
	_, err := jwthandle.Extract(errToken, request.SignatureKey)
	c.Assert(err, NotNil)
...
func (s *S) TestValidationWithWrongSignatureKey(c *C) {
	...
	_, err = jwthandle.Extract(token.GetStringToken(), errSignature)
	c.Assert(err, NotNil)
	c.Assert(err.Error(), Equals, "error parse token")
